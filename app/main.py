...
from app.cleanup import cleanup_after_successful_publish, cleanup_output
from app.r2_cleanup import run_r2_cleanup
...
def publish_scheduled_slot(slot: str | None = None) -> dict:
    ...
    append_slot_record(record)
    cleanup_after_successful_publish(record, requires_r2=instagram_enabled(), dry_run=False)
...
def main() -> None:
    ...
    parser.add_argument("--cleanup-output", action="store_true", help="Delete stale generated images under /app/output")
    parser.add_argument("--cleanup-output-dry-run", action="store_true", help="Show generated images that would be deleted")
    parser.add_argument("--r2-cleanup-dry-run", action="store_true", help="List old archived R2 media objects eligible for deletion")
    parser.add_argument("--r2-cleanup", action="store_true", help="Delete old archived R2 media objects under allowed prefixes")
    ...
    if args.content_schedule_status:
        ...
        return
    if args.cleanup_output_dry_run:
        deleted = cleanup_output(dry_run=True)
        print("cleanup_dry_run=true")
        print(f"cleanup_deleted_count={len(deleted)}")
        for path in deleted:
            print(f"cleanup_deleted={path}")
        return
    if args.cleanup_output:
        deleted = cleanup_output(dry_run=False)
        print("cleanup_dry_run=false")
        print(f"cleanup_deleted_count={len(deleted)}")
        for path in deleted:
            print(f"cleanup_deleted={path}")
        return
    if args.r2_cleanup_dry_run:
        result = run_r2_cleanup(dry_run=True)
        print("r2_cleanup_dry_run=true")
        if result.error:
            print(f"r2_cleanup_error={result.error}")
            raise SystemExit(1)
        print(f"r2_cleanup_keep_hours={os.getenv('R2_CLEANUP_KEEP_HOURS', '48')}")
        print(f"r2_cleanup_count={len(result.objects)}")
        print(f"r2_cleanup_bytes={result.total_bytes}")
        for item in result.objects:
            print(
                "r2_cleanup_would_delete="
                + f"{item['key']} size={item['size']} modified={item['last_modified']}"
            )
        return
    if args.r2_cleanup:
        result = run_r2_cleanup(dry_run=False)
        print("r2_cleanup_dry_run=false")
        if result.error:
            print(f"r2_cleanup_error={result.error}")
            raise SystemExit(1)
        print(f"r2_cleanup_deleted_count={len(result.deleted_keys)}")
        print(f"r2_cleanup_deleted_bytes={sum(int(item.get('size') or 0) for item in result.objects if item['key'] in result.deleted_keys)}")
        for key in result.deleted_keys:
            print(f"r2_cleanup_deleted={key}")
        return
