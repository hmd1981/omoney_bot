...
from app.cleanup import cleanup_after_successful_publish
...
def generate_content_preview(timezone: str) -> tuple[Path, dict, str]:
    ...
    caption = build_caption(theme, metadata=metadata)
    metadata["selected_cta"] = caption.split("\n\n")[1] if "\n\n" in caption else ""
...
def publish_content_post(
    *,
    ...
    theme = metadata.get("caption_theme") or next((item for item in THEMES if item["id"] == metadata["selected_theme"]), THEMES[0])
    caption = build_caption(theme, metadata=metadata)
    record["selected_cta"] = caption.split("\n\n")[1] if "\n\n" in caption else ""
    if metadata.get("selected_composition_profile"):
        record["selected_composition_profile"] = metadata["selected_composition_profile"]
    if metadata.get("rotation_bundle"):
        record["rotation_bundle"] = metadata["rotation_bundle"]
    ...
    append_content_record(record)
    cleanup_after_successful_publish(record, requires_r2=True, dry_run=False)
    return record
...
def publish_ai_content_post(
    *,
    ...
    caption = build_caption(theme, metadata=record)
    record["selected_cta"] = caption.split("\n\n")[1] if "\n\n" in caption else ""
