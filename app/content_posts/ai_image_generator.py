...
from app.content_posts.composition_profiles import COMPOSITION_PROFILES
from app.content_posts.content_rotation import build_rotation_bundle, rotation_prompt_suffix
from app.content_posts.rotation_memory import pick_theme, record_theme, record_visual
...
def choose_ai_theme(seed: int | None = None) -> dict:
    ...
    return pick_theme(AI_PROMPT_THEMES, lambda item: str(item["id"]), rng)


def build_openai_prompt(theme: dict, rotation_bundle: dict[str, str] | None = None) -> str:
    rotation_text = rotation_prompt_suffix(rotation_bundle) if rotation_bundle else ""
    return (
        ...
        f"Scene theme: {theme['prompt']}. {rotation_text} {NEGATIVE_PROMPT}"
    )
...
def apply_brand_overlay(
    ...
    tagline: str,
    output_path: Path,
    composition_profile_id: str = "luxury_finance",
) -> None:
    profile = COMPOSITION_PROFILES.get(composition_profile_id, COMPOSITION_PROFILES["luxury_finance"])
    scales = profile.get("typography_scale", {})
    shade = Image.new("RGBA", image.size, (0, 0, 0, 0))
    sd = ImageDraw.Draw(shade)
    sd.rectangle((0, 0, WIDTH, SAFE_PAD_Y + 118), fill=(0, 0, 0, int(profile.get("shade_top_alpha", 126))))
    body_alpha = int(profile.get("shade_body_alpha", 0))
    if body_alpha > 0:
        sd.rectangle((SAFE_PAD_X, SAFE_PAD_Y, WIDTH - SAFE_PAD_X, HEIGHT - SAFE_PAD_Y), fill=(0, 0, 0, body_alpha))
    outline_alpha = int(profile.get("outline_alpha", 156))
    sd.rounded_rectangle(
        (SAFE_PAD_X, SAFE_PAD_Y, WIDTH - SAFE_PAD_X, HEIGHT - SAFE_PAD_Y),
        radius=int(profile.get("border_radius", 32)),
        outline=(231, 193, 111, outline_alpha),
        width=2,
    )
    image.alpha_composite(shade)
    image = image.filter(ImageFilter.UnsharpMask(radius=1.0, percent=110, threshold=3))
    draw = ImageDraw.Draw(image)
    brand_font = _font(int(scales.get("brand", 34)), True, True)
    title_font = _font(int(scales.get("title", 23)), True)
    tag_font = _font(int(scales.get("tag", 19)), False)
    top_y = SAFE_PAD_Y + 34
    left_x = SAFE_PAD_X + 34
    right_x = WIDTH - SAFE_PAD_X - 34
    draw.text((left_x, top_y), "OMoney", font=brand_font, fill=GOLD)
    title_y = top_y + int(profile.get("title_offset_y", 6))
    tag_y = top_y + int(profile.get("tag_offset_y", 40))
    _draw_right(draw, headline, right_x, title_y, title_font, WHITE)
    _draw_right(draw, tagline, right_x, tag_y, tag_font, MUTED)
...
def generate_ai_content_image(image_path: Path, timezone: str = "Asia/Muscat", seed: int | None = None) -> dict:
    ...
    rotation_bundle = build_rotation_bundle(rng)
    composition_profile_id = str(rotation_bundle.get("composition_profile", "luxury_finance"))
    raw_path = image_path.with_name(image_path.stem + "-openai-raw.png")
    prompt = build_openai_prompt(theme, rotation_bundle)
    ...
    record_visual(f"openai_{composition_profile_id}")
    ...
    return {
        ...
        "selected_visual_preset": f"openai_realistic_{composition_profile_id}",
        "selected_composition_profile": composition_profile_id,
        "rotation_bundle": rotation_bundle,
